...
b1<- read.table("./ginsimout_Metastasis_edges_selected_stringent3.txt", header = TRUE, sep="\t", stringsAsFactors=FALSE)
...
S<-rbind(PCcat[PCcat$TYPE=="SINGLE" & (PCcat$PC1>radius | PCcat$PC1 <= -radius) & (PCcat$PC2>radius | PCcat$PC2<= -radius),],PCcat[grepl(paste0(pattern,collapse = "|"), rownames(PCcat)),])
S$col<-"dark red"
...
p<-ggplot()+ theme_bw(base_size = 20) +
  ...
  geom_text_repel(data=vPCs2, aes(x=vPCs2$PC1,y=vPCs2$PC2,
                                  # label=gsub("-","-\n-",rownames(vPCs2),perl=T)
                                  label=c("HS",
                                          "Apoptosis\n--CellCycleArrest",
                                          "EMT\n--CellCycleArrest",
                                          "Migration--Metastasis\n--Invasion--EMT\n--CellCycleArrest")
                                  ), size=5, colour="black",segment.color=NA)+
  geom_text_repel(data=S_epis, aes(x=S_epis$PC1,y=S_epis$PC2,label=rownames(S_epis)), size=4.7, colour=S_epis$col,
...
p2<-ggplot()+ theme_bw(base_size = 20) +
  ...
  scale_y_continuous(limits=c(-6, 6)) +
  coord_fixed(ratio = 1) +
  # +  theme(axis.title.x = element_text(size = 15, vjust=-.2), axis.title.y = element_text(size = 15, vjust=0.3))
